...
describe("Test /api/search/course", () => {
  it("Should return courses by course code", () => {
    const queryText = "CIS";
    cy.request("GET", `/api/search/course?query=${queryText}`).then(({ status, body }) => {
  ...
  it("Should return courses by course name", () => {
    ...
    cy.request("GET", `/api/search/course?query=${queryText}`).then(({ status, body }) => {
      const data = JSON.parse(body);
      expect(status).to.equal(200);
      expect(data.every((course: Course) => course.name.toLowerCase().includes(queryText))).to.be.true;
    });
  });
});
