...
(async () => {
  ...
  await session.run(`
      MATCH (e)
      WHERE (e:OrBlock or e:Seminar or e:Tutorial or e:Lab or e:CreditRequirement or e:RegistrationRequirement or e:Section or e:Lecture or e:AndBlock)
      DETACH DELETE e
    `);
  await session.close();
            ...
            `,
                {
                  meetings: section[meetingType].map((e) => ({
                    ...
                    seatsAvailable: isNaN(parseInt(e.seatsAvailable)) ? 0 : e.seatsAvailable,
                  })),
              ...
              log(chalk.blueBright(`Added ${meetingType} for section ${section.code}`));
            } catch (error) {
    ...
    try {
      ...
      await session.run(
        `
          ...
          ON CREATE
            SET program.id = $programId
            SET program.name = $name
            SET program.updatedAt = timestamp()

          MERGE (school)-[:OFFERS]->(program)
        `,
          ...
          if (blockCourses.length > 1) {
            ...
            await session.run(
              `
                MATCH (program:Program {id: $programId})
                MATCH (course:Course {code: $code})
                MERGE (program)-[:MAJOR_REQUIRES]->(course)
              `,
          ...
          if (blockCourses.length > 1) {
            ...
            await session.run(
              `
                MATCH (program:Program {id: $programId})
                MATCH (course:Course {code: $code})
                MERGE (program)-[:MINOR_REQUIRES]->(course)
              `,
              { code: course.replace("*", ""), programId }
            );
            await session.close();
  ...
  try {
    ...
    log("Added programs to program index.");
  } catch (error) {
    log(chalk.red(error));
  }
