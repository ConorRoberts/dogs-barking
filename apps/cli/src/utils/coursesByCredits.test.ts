...
import coursesByCredits from ".";

const testCredits = 0.75;
const absentCredits = 1;
...
test("Checks if existing credit value returns courses", () => {
    expect(coursesByCredits(testCourses, testCredits)).toEqual(expectedCourses);
});

test("Checks if absent credit value returns no courses", () =>{
    expect(coursesByCredits(testCourses, absentCredits)).toEqual([]);
});