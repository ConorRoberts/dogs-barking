import compareCourseCodeDesc from ".";
...
test("courseA greater than courseB", () => {
    expect(compareCourseCodeDesc(mockACCT1220, mockCIS3760)).toBeGreaterThan(0);
});
test("courseA less than courseB", () => {
    expect(compareCourseCodeDesc(mockCIS3760, mockACCT1220)).toBeLessThan(0);
});
test("courseA equal to courseB", () => {
    expect(compareCourseCodeDesc(mockCIS3760, mockCIS3760)).toEqual(0);
});