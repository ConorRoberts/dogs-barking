...
test("courseA less than courseB", () => {
    expect(compareCourseCode(mockACCT1220, mockCIS3760)).toBeLessThan(0);
});
test("courseA greater than courseB", () => {
    expect(compareCourseCode(mockCIS3760, mockACCT1220)).toBeGreaterThan(0);
});
test("courseA equal to courseB", () => {
    expect(compareCourseCode(mockCIS3760, mockCIS3760)).toEqual(0);
});