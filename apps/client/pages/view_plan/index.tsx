  ...
  return (
    <>
                      ...
                      <div className="place-content-center p-2 m-4 bg-neutral-200 rounded-3xl">
                        ...
                        <ul className="bg-white rounded-r-2xl rounded-l-lg pb-1" key={Math.random()}>
                          {semester.courses.map((course) => {
                            return(
                              <li className="indent-6 pt-1 pr-1" key={Math.random()}>{course.id} - {course.name} ({course.weight} credits)</li>
                            );
                          })}
                        </ul>
                      </div>
      ...
      {!isPlannerStateEmpty() ? 
        (
          <>
            <Button className={showPrintHelp ? "w-30 place-self-center rounded-md mt-8 bg-blue-500 hover:bg-blue-700" : "w-30 place-self-center rounded-md mt-8 bg-green-500 hover:bg-green-700"} onClick={() => setShowPrintHelp(!showPrintHelp)}>
              {showPrintHelp ? "Hide Print Help" : "Show Print Help"}
            </Button>
            <footer className={showPrintHelp ? "flex flex-col w-full" : "hidden"}>
              <p className="italic place-self-center mt-4 text-lg">Looking to print this page? Try pressing CTRL+P on Windows or Command+P on Mac</p>
            </footer> 
          </>
        ) : null
      }
