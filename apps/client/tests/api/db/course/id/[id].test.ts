...
import handler from "../../../../../pages/api/db/course/id/[id]";
import httpMocks from "node-mocks-http";
import { NextApiRequest, NextApiResponse } from "next";

test("Fetch course with Guelph Course ID", async () => {
  const req = httpMocks.createRequest<NextApiRequest>({query: {id: "ACCT1220"}});
...
test("Fetch course with UofT Course ID", async () => {
  const req = httpMocks.createRequest<NextApiRequest>({query: {id: "STA305H1"}});
  const res = httpMocks.createResponse<NextApiResponse>();
  await handler(req, res);
...
test("Fetch course with Invalid Course ID", async () => {
  ...
  expect(res._getJSONData().message).toBe("Invalid Course ID");
